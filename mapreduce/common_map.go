...
func doMap(
	...
	temp := make([]*os.File, nReduce)
	enc := make([]*json.Encoder, nReduce)
	for i := 0; i < nReduce; i++ {
		fn := reduceName(jobName, mapTaskNumber, i)
		temp[i], _ = os.OpenFile(fn, os.O_RDWR|os.O_CREATE, 0666)
		enc[i] = json.NewEncoder(temp[i])
	}

	/*     Use user-defined map Funciont to get key-value pairs */
	valuePairs := mapF(inFile, string(content))

	/*  Use ihash function to get intermediate partition */
	for _, value := range valuePairs {
		enc[myhash(value.Key)%nReduce].Encode(&value)
	}
